...
func (s *sqlStore) initTrackerStatements() error {
	var err error
...
func (s *sqlStore) CreateTracker(tracker *model.Tracker, userID int64) (int64, error) {
	tx, err := s.db.Begin()
	if err != nil {
	...
	row := tx.Stmt(s.trackerStatements.create).QueryRow(
...
func (s *sqlStore) GetTracker(trackerID int64) (*model.Tracker, error) {
...
func (s *sqlStore) GetTrackerByUserID(trackerID int64, userID int64) (*model.Tracker, error) {
...
func (s *sqlStore) UpdateTracker(tracker *model.Tracker, userID int64) error {
	tx, err := s.db.Begin()
	if err != nil {
...
func (s *sqlStore) DeleteTracker(trackerID int64, userID int64) error {
	tx, err := s.db.Begin()
	if err != nil {
		return errors.NewStorageErrorFromError(err)
	}
...
func (s *sqlStore) ListTrackers(offset int64, limit int64) ([]model.Tracker, error) {
...
func (s *sqlStore) ListTrackersByCollectionID(collectionID int64, userID int64, offset int64, limit int64) ([]model.Tracker, error) {
	tx, err := s.db.Begin()
	if err != nil {
